...
// RPP NOTES: header taken directly from https://github.com/eranpeer/FakeIt/blob/dev/single_header/standalone/fakeit.hpp
// FakeIt itself uses MIT license https://github.com/eranpeer/FakeIt/blob/dev/LICENSE
// Smal changes were done in this header: FakeItException is now derived from std::runtime_error
...
namespace fakeit
{
    ...
    public:
        static const bool value =
            std::is_arithmetic<T>::value || std::is_pointer<T>::value || std::is_same<decltype(test(*(std::ostream*)nullptr, std::declval<T>())), std::ostream&>::value;
    };
...
namespace fakeit
{
    ...
    struct FakeitContext : public EventHandler
...
namespace fakeit
{
    ...
    struct FakeitException : public std::runtime_error
    {
    ...
    {
        using FakeitException::FakeitException;
    };
...
namespace fakeit
{
    ...
    public:
        NoVirtualDtor()
            : std::runtime_error("Can't mock the destructor. No virtual destructor was found")
        {
        }
    };
...
namespace fakeit
{
    ...
    struct TupleDispatcher
    {
        ...
        static void for_each(TupleType1&&, TupleType2&&, FunctionType&, std::integral_constant<size_t, std::tuple_size<typename std::remove_reference<TupleType1>::type>::value>)
        {
        }
...
namespace fakeit
{
    ...
    template<typename T, typename U, typename std::enable_if<std::is_arithmetic<typename naked_type<T>::type>::value, int>::type = 0, typename std::enable_if<std::is_arithmetic<typename naked_type<U>::type>::value, int>::type = 0>
...
namespace fakeit
{
    ...
    namespace placeholders
    {
        ...
        template<typename PlaceHolder, typename ArgType, typename std::enable_if<static_cast<bool>(std::is_placeholder<PlaceHolder>::value), bool>::type = true>
...
namespace fakeit
{
    ...
    public:
        ...
        typename std::enable_if<
            !std::is_same<AnyMatcher, typename naked_type<Head>::type>::value && !IsMatcherCreatorTypeCompatible<typename naked_type<Head>::type>::value && std::is_constructible<NakedArgType<index>, Head&&>::value,
            void>::type
...
namespace fakeit
{
    ...
    protected:
        ...
        typename std::enable_if<
            sizeof...(matcherCreators) == sizeof...(arglist),
            void>::type
...
namespace fakeit
{
    ...
    public:
        ...
        template<class DataType, typename... arglist, class = typename std::enable_if<std::is_member_object_pointer<DataType C::*>::value>::type>
...
namespace fakeit
{
    ...
    private:
        ...
        static bool findNextMatch(std::vector<Sequence*>& pattern, std::vector<Invocation*>& actualSequence, int startSearchIndex, int& end, std::vector<Invocation*>& matchedInvocations)
        {
